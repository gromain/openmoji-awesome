...
fun openmojiColorUrl(unicode: String) = "https://cdn.jsdelivr.net/gh/hfg-gmuend/openmoji@14.0.0/color/svg/$unicode.svg"
fun openmojiBlackUrl(unicode: String) = "https://cdn.jsdelivr.net/gh/hfg-gmuend/openmoji@14.0.0/black/svg/$unicode.svg"
...
fun main(args: Array<String>) {
    ...
    val json = URL("https://raw.githubusercontent.com/hfg-gmuend/openmoji/14.0.0/data/openmoji.json").readText()
    val result = Klaxon().parseArray<Emoji>(json) ?: listOf()
    val new = result.filter {
        it.annotation.all { it.isLetter() || it == ' ' || it == '-' || it == ':' || it == '(' || it == ')'}
    }.map {
        Emoji(it.hexcode.toUpperCase().split(" ").joinToString("-"),
              it.annotation.toLowerCase().split(" ").joinToString("-").replace(":", "").replace("(", "").replace(")", ""))
    }
